...
import { randomColors } from '../utils/helper';

const ChatMessage = ({name,message}) => {
    const color = randomColors();
  return (
		<div>
			<div className="flex p-1 shadow-md bg-gray-100 mb-1">
				<svg
				...
				<p className={`mt-2 px-1 font-semibold ${color}`}>{name} :</p>
				<p className="mt-2 px-1">{message}</p>
			</div>
		</div>
	);
}
